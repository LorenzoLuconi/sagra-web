...
import {
  fetchProductDelete, productsSearchQuery
              ...
              {products?.map((product: Product) => {
                return (
                  <TableRow key={product.id} selected={props.selected?.id === product.id}>
                    ...
                    <TableCell sx={{ fontSize: "1.0em" }}><DepartmentName departmentId={product.departmentId} /></TableCell>
                    <TableCell align='right' sx={{ fontSize: "1.0em" }}>{currency(product.price)}</TableCell>
                    <TableCell sx={{ fontSize: "0.9em"}}>{product.parentId ? <><LinkOutlined sx={{mr: 0.8, verticalAlign: 'middle'}}/><ProductName  productId={product.parentId} /></> : ''}</TableCell>
